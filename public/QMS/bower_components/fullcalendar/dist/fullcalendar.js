...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderSlatRowHtml: function() {
		...
		while (slotTime < view.maxTime) {
			...
			html +=
				'<tr id="'+slotDate.format('HH:mm:ss')+'"  data-time="' + slotDate.format('HH:mm:ss') + '"' +
					(isLabeled ? '' : ' class="fc-minor"') +
					'>' +
					(!isRTL ? axisHtml : '') +
					'<td class="' + theme.getClass('widgetContent') + '"><a style="cursor: pointer;" onclick="bookSlot(\''+slotDate.format('HH:mm:ss')+'\');"><i class="fa fa-plus-circle" style="height: 20%;"></i></a></td>' +
					(isRTL ? axisHtml : '') +
				"</tr>";
...
Calendar.defaults = {
	...
	// display : either ( month,agendaWeek(Week),agendaDay(Day) )
